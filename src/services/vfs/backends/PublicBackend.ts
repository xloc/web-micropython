...
// Vite's import.meta.glob discovers all .pyi files at build time
...
export class PublicBackend implements VFSBackend {
  ...
  private toPublicUrl(path: string): string {
    ...
    return path.replace('/stubs', '/micropython-stubs')
  }
  ...
  async readdir(path: string): Promise<VFSFile[]> {
    if (!this.dirList.has(path) && path !== '/stubs') {
    ...
    for (const filePath of this.fileList) {
      if (filePath.startsWith(normalizedPath + '/') || (normalizedPath === '/stubs' && filePath.startsWith('/stubs/'))) {
